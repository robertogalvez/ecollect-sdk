...
const captureResult = await client.payments.capture(ticketId, 45000);
...
try {
  ...
  if (err instanceof InvalidCardException) {
    ...
    console.error('Insufficient funds — ask user to try another card');
...
It is your internal order/reference ID. ecollect stores it alongside the transaction so you can find it later. It **must be unique per transaction** — if you reuse one, you will get a `DuplicateTransactionException`.
